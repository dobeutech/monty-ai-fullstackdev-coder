...
${colors.bright}ENVIRONMENT:${colors.reset}
  ${colors.yellow}ANTHROPIC_API_KEY${colors.reset}          Required for Claude API access
...
async function main() {
  ...
  // Run the main agent
  const child = spawn(runner, [...runnerArgs.slice(0, 1), ...transformedArgs.filter(a => !['init', 'code', 'status', 'setup'].includes(a))], {
    stdio: 'inherit',
    cwd: process.cwd(),
    env: process.env,
  });
